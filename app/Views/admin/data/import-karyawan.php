                ...
                <div class="row">
                                    ...
                                    <div class="col-sm-12">
                                        <div class="row">
                                            <div id="csv_upload_spinner" class="csv-upload-spinner">
                                                <strong class="text-csv-importing">Importing Karyawan...</strong>
                    ...
                    <div class="col-12 col-xl-4">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form action="<?= base_url('admin/karyawan/downloadCSVFilePost'); ?>" method="post">
                                    <?= csrf_field(); ?>
                                    <button type="button" class="btn btn-success btn-block" data-toggle="modal" data-target="#modalDepartemen">List Departemen</button>
                                    <button class="btn btn-success btn-block" name="submit" value="csv_karyawan_template">Download CSV Template</button>
                                </form>
                            </div>
                        </div>
                    </div>
...
<div class="modal fade" id="modalDepartemen" tabindex="-1" aria-labelledby="modaldepartemenTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalDepartemenTitle">List Departemen</h5>
...
<script>
    $(function() {
        $('#drag-and-drop-zone').dmUploader({
            url: '<?= base_url("admin/karyawan/generateCSVObjectPost"); ?>',
            multiple: false,
            extFilter: ["csv"],
    ...
    function addCSVItem(numberOfItems, txtFileName, index) {
        if (index <= numberOfItems) {
            ...
            $.ajax({
                type: "POST",
                url: '<?= base_url("admin/karyawan/importCSVItemPost"); ?>',
                data: setAjaxData(data),
                success: function(response) {
                    var objSub = JSON.parse(response);
                    if (objSub.result == 1) {
                        $("#csv_uploaded_files").prepend('<li class="list-group-item list-group-item-success">&nbsp;' + objSub.index + '.&nbsp;' + objSub.karyawan.nis + '.&nbsp; - ' + objSub.karyawan.nama_karyawan +'</li>');
                    } else {
