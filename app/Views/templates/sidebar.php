...
// Flag untuk animasi tile aktif di sidebar
$isApprovalActive = ($context === 'approval');
...
<style>
  /* Sidebar active tile animation (teal like example) */
  @keyframes sidebarTilePop {
    0% {
  ...
  .sidebar .nav .nav-item.active-animated>.nav-link {
    background: #06b6d4 !important;
    /* teal */
    border-radius: 12px;
    animation: sidebarTilePop .38s ease-out 1;
    box-shadow: 0 10px 22px rgba(6, 182, 212, .22);
  }

  .sidebar .nav .nav-item.active-animated>.nav-link i.material-icons,
  .sidebar .nav .nav-item.active-animated>.nav-link p {
  ...
  <div class="sidebar-wrapper">
    <ul class="nav">
      ...
      <?php if ($accessibleMenus['approval_management'] ?? false): ?>
        <li class="nav-item <?= nav_active($context, 'approval', ['admin/approval*']) ?> <?= $isApprovalActive ? 'active-animated' : '' ?>">
